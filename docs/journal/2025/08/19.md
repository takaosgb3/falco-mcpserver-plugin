...
### 本日の追加更新（ドキュメントのみ・実装着手なし）
- NEXT_ACTIONS にフェーズ番号（P2-*.**）を併記（方針B）。NX⇔P2 の対応を明確化。
- MCP テストサーバ設計に WS モード最小仕様の付録を追記（フレーム/上限/接続数/JSON-RPC扱い）。
- CI 設計に Step Summary/Artifacts の具体仕様を追記（出力フォーマット・命名規約・収集設定）。
- Config ガイドにプライバシー/保持/オプトアウト/レッドアクトの推奨を追記。

### 合意・意思決定
- タスク管理: `NEXT_ACTIONS` は NX に P2 番号を併記（B方針）。
- WS モード: Phase 2 は `ws://` の最小実装を優先。`wss://` は将来（観測は `--assume-tls` で代替）。
- `.method` 粒度: 既定OFF、フラグでON（per-callは任意機能）。
- 追加依存: 導入しない（WSも最小実装方針）。

### 倫理・セキュリティ観点の再確認
- 収集最小化（メタデータ中心、本文/認証情報は収集しない）。
- 透明性/オプトアウト（監査は明示起動時のみ有効、将来は環境変数トグルも検討）。
- 保存/権限（0600、短期保持/ローテーション推奨、外部公開禁止）。

### 明日の候補タスク（着手前に最終確認）
- NX-1/P2-1.2: `mcp-test-server --mode ws` の最小実装（ループバックlisten、テキストフレーム、サイズ上限）。
- NX-2/P2-2.2: WS 統合テストの骨子（SKIP条件・成果物設計）を追加。
- NX-5/P2-4: Step Summary 集計出力の最小実装（件数カウント）。

### ✅ 明日のToDo（忘れないチェックリスト）
- [ ] NX-1/P2-1.2: `mcp-test-server --mode ws` を実装し `go build` が通ることを確認（単発呼出で200ms未満応答）。
- [ ] NX-2/P2-2.2: `mcp-audit-proxy` 経由のWS統合テストを作成（監査JSONに `server_host`/`tls=true`/`bytes>0` を確認）。
- [ ] NX-5/P2-4: CI Step Summary に PASS/FAIL/SKIP を集計して出力。
- [ ] 余力: `.method` 抽出のフラグ実装方針を具体化（仕様のみでも可）。
